...
int main()
{
	...
	ret = Read(fd, buf, SIZE);
	assert("Read wrong c_item_num of bytes: 000\n", ret == SIZE);
	...
	ret = Read(fd, buf, SIZE);
	assert("Read wrong c_item_num of bytes: FFF\n", ret == SIZE);
	assert("Failed read on file: FFF\n", strncmp(buf, fff, SIZE) == 0);

	ret = Seek(fd, 0, SEEK_END);
	ret = Read(fd, buf, SIZE);
	assert("Read wrong c_item_num of bytes: seek end\n", ret == 0);
	Close(fd);
	...
	fd = Open(".");
	ret = Read(fd, buf, 16);
	assert("Read wrong c_item_num of bytes: . dir\n", ret == 16);
	ret = Write(fd, buf, 16);
